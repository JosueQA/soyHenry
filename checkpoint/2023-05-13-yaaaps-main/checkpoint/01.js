...
function henryHospital(lista) {
  // Tu código aquí:
  let atendidos = new Queue();
  let espera = new Queue();
  let result = {};
  for (var i = 0; i < lista.array.length; i++) {
    if (
      lista.array[i].edad >= 18 &&
      lista.array[i].altura > 150
    ) {
      atendidos.enqueue(lista.array[i]);
    } else {
      espera.enqueue(lista.array[i]);
    }
  }
  result['Atendidas'] = atendidos;
  result['EnEspera'] = espera;
  return result;
}
